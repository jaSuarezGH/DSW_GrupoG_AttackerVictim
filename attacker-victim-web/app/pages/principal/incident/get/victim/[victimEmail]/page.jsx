...
export default async function getIncidentByVictimEmailPage({ params }) {
  ...
  try {
    ...
    const victim = await fetchGetDelete(endGetVictimById, user.id);

    if (!victim) {
      throw new Error("Victima no encontrada");
    }

    const incident = await fetchGetDelete(endGetIncidentByVictim, victim.id);
      ...
      <>
        <div className="max-w-6xl mb-6 ring-1 ring-opacity-50 ring-zinc-300 rounded-xl shadow-lg shadow-indigo-300 mt-6 flex min-h-full flex-1 flex-col justify-center align-middle px-6 py-6 lg:px-8 mx-auto gap-x-8 gap-y-12">
          ...
          <DivSubHeader
            ...
            description={`Datos correspondientes al Usuario Victima consultado.`}
          ></DivSubHeader>
          <ListConsultaUser user={incident._victim._user}></ListConsultaUser>
          <DivSubHeader
            title={`Datos del Usuario Atacante`}
            description={`Datos correspondientes al Usuario Atacante relacionado al Victima.`}
          ></DivSubHeader>
