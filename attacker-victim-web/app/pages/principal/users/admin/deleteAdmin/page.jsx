...
import { optionsConsultAdmin } from "../models/optionsConsultAdmin.model";
...
export default function deleteUsersPage() {
  const [selected, setSelected] = useState(optionsConsultAdmin[0]);
  ...
  const onSubmit = (e) => {
    ...
    switch (selected) {
      case "Nombre de Usuario (Username)":
        router.push(`${Routes.DELETE_ADMIN_USERNAME}${valor}`);
        break;

      case "Correo Electronico (Email)":
        router.push(`${Routes.DELETE_ADMIN_EMAIL}${valor}`);
        break;
    }
  };
  ...
  return (
    <>
      <div className="ring-1 ring-opacity-40 ring-zinc-300 rounded-xl shadow-md shadow-indigo-100 mt-6 flex min-h-full flex-1 flex-col justify-center align-middle px-6 py-6 lg:px-8 mx-auto max-w-7xl gap-x-8 gap-y-12">
        <DivHeader
          title="Eliminar a un Usuario Administrador"
          description="Eliminar a un usuario de tipo Administrador registrado en el sistema."
          tags={[2, 3]}
        ></DivHeader>
        <div className="ring-1 ring-opacity-70 ring-zinc-300 shadow-lg shadow-indigo-300 mb-6 rounded-lg p-9 sm:mx-auto sm:w-full sm:max-w-sm">
          <form className="space-y-6 " onSubmit={onSubmit}>
            ...
            <ButtonSubmit text="Buscar Administrador a Eliminar"></ButtonSubmit>
          </form>
        </div>
      </div>
    </>
