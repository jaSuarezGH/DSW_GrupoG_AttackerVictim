...
export default function UpdateEmailPage({ params }) {
  ...
  useEffect(() => {
    const fetchDataUser = async () => {
      ...
      const userFetch = await fetchGetDelete(endGetUserByEmail, params.email);
      setUser(userFetch);
  ...
  if (user === null && control) {
    const description = `Lo siento, el usuario a modificar poseedor del Correo Electronico (Email): "${decodeURIComponent(params.email)}" no se encuentra registrado.`;
    return (
      <InformacionPage
