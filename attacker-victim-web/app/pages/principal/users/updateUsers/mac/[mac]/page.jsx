...
export default function UpdateMACPage({ params }) {
  ...
  useEffect(() => {
    const fetchDataUser = async () => {
      ...
      const userFetch = await fetchGetDelete(endGetUserByMAC, params.mac);
      setUser(userFetch);
  ...
  if (user === null && control) {
    const description = `Lo siento, el usuario a modificar poseedor de la Direccion MAC: "${params.mac}" no se encuentra registrado.`;
    return (
      <InformacionPage
